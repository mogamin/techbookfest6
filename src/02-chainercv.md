...
上記のように表示されます。
例えば、左端は赤を表すチャネルの単色画像で、ロゴがうっすらと見えるかと思います。これは普通の赤よりも濃い赤が使用されていることを表しています。そして、中と右端の画像ではロゴが真っ黒になっています。これは、ロゴの部分に緑と青の要素が弱いことを表しています。
...
これまでに紹介した`random_crop`,`random_flip`,`random_rotate`,`random_expand`をそれぞれランダムに実行してみるとどうなるでしょう。先程のコードの続きで実行してみてください。
